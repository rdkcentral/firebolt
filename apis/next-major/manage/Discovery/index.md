...
Version Discovery 1.8.0-next-major.4
...
```typescript
function listen('onSignIn', (object) => void): Promise<number>
```
...
let listenerId = await Discovery.listen('onSignIn', (result) => {
  console.log(result)
})
```
...
```typescript
function listen('onSignOut', (object) => void): Promise<number>
```
...
let listenerId = await Discovery.listen('onSignOut', (result) => {
  console.log(result)
})
```
