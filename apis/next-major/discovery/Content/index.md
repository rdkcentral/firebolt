...
Version Content 1.8.0-next-major.2
...
| -------- | -------------------------- | -------- | ------------------------------------------------------ |
| `type`   | `Discovery.InterestType`   | true     | <br/>values: `'interest' \| 'disinterest'`             |
| `reason` | `Discovery.InterestReason` | true     | <br/>values: `'playlist' \| 'reaction' \| 'recording'` |
...
| `type`     | `Discovery.InterestType`   | true     | <br/>values: `'interest' \| 'disinterest'`             |
| `reason`   | `Discovery.InterestReason` | true     | <br/>values: `'playlist' \| 'reaction' \| 'recording'` |
| `interest` | `Discovery.InterestResult` | false    | The EntityDetails data.                                |
...
let listenerId = await Content.listen(
...
let listenerId = await Content.listen('requestUserInterestChanged', result => {
  console.log(result)
})
````
...
```typescript
function listen('requestUserInterestChanged', type: InterestType, reason: InterestReason, (InterestResult) => void): Promise<number>
````
