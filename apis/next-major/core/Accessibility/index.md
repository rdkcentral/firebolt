...
Version Accessibility 1.8.0-next-major.4
...
- [Events](#events)
  - [onAudioDescriptionSettingsChanged](#onaudiodescriptionsettingschanged)
  - [onClosedCaptionsSettingsChanged](#onclosedcaptionssettingschanged)
  - [onHighContrastUIChanged](#onhighcontrastuichanged)
  - [onVoiceGuidanceSettingsChanged](#onvoiceguidancesettingschanged)
...
function audioDescriptionSettings(
  callback: (settings) => AudioDescriptionSettings,
): Promise<number>
```

| Param      | Type                                                      | Required | Description                    |
| ---------- | --------------------------------------------------------- | -------- | ------------------------------ |
| `settings` | [`AudioDescriptionSettings`](#audiodescriptionsettings-1) | false    | the audio description settings |
...
let listenerId = await audioDescriptionSettings((result) => {
  console.log(result)
})
```
...
[Deprecated] This method is deprecated as of since version 0.6.0. Please use `closedCaptionsSettings` as a replacement.
...
```javascript
{"enabled":true,"styles":{"fontFamily":"monospaced_sanserif","fontSize":1,"fontColor":"#ffffff","fontEdge":"none","fontEdgeColor":"#7F7F7F","fontOpacity":100,"backgroundColor":"#000000","backgroundOpacity":100,"textAlign":"center","textAlignVertical":"middle","windowColor":"white","windowOpacity":50},"preferredLanguages":["eng","spa"]}
```
...
function closedCaptionsSettings(
  callback: (closedCaptionsSettings) => ClosedCaptionsSettings,
): Promise<number>
```

| Param                    | Type                                                                         | Required | Description                  |
| ------------------------ | ---------------------------------------------------------------------------- | -------- | ---------------------------- |
| `closedCaptionsSettings` | [`ClosedCaptionsSettings`](../Accessibility/schemas/#ClosedCaptionsSettings) | false    | the closed captions settings |
...
let listenerId = await closedCaptionsSettings((result) => {
  console.log(result)
})
```
...
```typescript
function highContrastUI(callback: (highContrastUI) => boolean): Promise<number>
```

| Param            | Type      | Required | Description                              |
| ---------------- | --------- | -------- | ---------------------------------------- |
| `highContrastUI` | `boolean` | false    | Whether high-contrast UI mode is enabled |
...
let listenerId = await highContrastUI((result) => {
  console.log(result)
})
```
...
[Deprecated] This method is deprecated as of since version 0.6.0. Please use `voiceGuidanceSettings` as a replacement.
...
function voiceGuidanceSettings(
  callback: (settings) => VoiceGuidanceSettings,
): Promise<number>
```

| Param      | Type                                                                       | Required | Description                 |
| ---------- | -------------------------------------------------------------------------- | -------- | --------------------------- |
| `settings` | [`VoiceGuidanceSettings`](../Accessibility/schemas/#VoiceGuidanceSettings) | false    | the voice guidance settings |
...
let listenerId = await voiceGuidanceSettings((result) => {
  console.log(result)
})
```
...
### onAudioDescriptionSettingsChanged
