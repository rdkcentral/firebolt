...
Version Device 1.8.0-next-major.4
...
- [Events](#events)
  ...
  - [onHdcpChanged](#onhdcpchanged)
  - [onHdrChanged](#onhdrchanged)
  - [onNameChanged](#onnamechanged)
  - [onNetworkChanged](#onnetworkchanged)
  - [onScreenResolutionChanged](#onscreenresolutionchanged)
...
```typescript
function audio(
  callback: (supportedAudioProfiles) => AudioProfiles,
): Promise<number>
```
...
let listenerId = await audio((result) => {
  console.log(result)
})
```
...
```typescript
function hdcp(
  callback: (negotiatedHdcpVersions) => HDCPVersionMap,
): Promise<number>
```
...
let listenerId = await hdcp((result) => {
  console.log(result)
})
```
...
let listenerId = await name((result) => {
  console.log(result)
})
```
...
```typescript
function network(callback: (networkInfo) => NetworkInfoResult): Promise<number>
```
...
let listenerId = await network((result) => {
  console.log(result)
})
```
...
```typescript
function  W3C APIs(callback: (screenResolution) => [
    720, // undefined Width in pixels item
    480 // undefined Height in pixels item
] | [
    720, // undefined Width in pixels item
...
let listenerId = await  W3C APIs(result => {
  console.log(result)
})
...
let listenerId = await videoResolution((result) => {
  console.log(result)
})
```
