...
```typescript
function ${method.alternative}(${event.signature.params}${if.context}, ${end.if.context}callback: (value) => AudioProfiles): Promise<number>
```
...
```typescript
function ${method.alternative}(${event.signature.params}${if.context}, ${end.if.context}callback: (value) => HDCPVersionMap): Promise<number>
```
...
```typescript
function ${method.alternative}(${event.signature.params}${if.context}, ${end.if.context}callback: (value) => HDRFormatMap): Promise<number>
```
...
```typescript
function ${method.alternative}(${event.signature.params}${if.context}, ${end.if.context}callback: (value) => NetworkInfoResult): Promise<number>
```
...
```typescript
function ${method.alternative}(${event.signature.params}${if.context}, ${end.if.context}callback: (value) => [
    720, // undefined Width in pixels item
    480 // undefined Height in pixels item
] | [
    720, // undefined Width in pixels item
...
Device.listen('audioChanged', (result) => {
  console.log(result)
})
```
...
Device.listen('hdcpChanged', (result) => {
  console.log(result)
})
```
...
Device.listen('hdrChanged', (result) => {
  console.log(result)
})
```
...
Device.listen('networkChanged', (result) => {
  console.log(result)
})
```
...
Device.listen('videoResolutionChanged', (result) => {
  console.log(result)
})
```
...
```typescript
type DeviceVersion = {
  sdk?: Types.SemanticVersion // The Firebolt SDK version
  api: Types.SemanticVersion // The latest Firebolt API version supported by the current device.
  firmware: Types.SemanticVersion // The firmware version as reported by the device
  os: Types.SemanticVersion // **Deprecated** Use `firmware`, instead.
