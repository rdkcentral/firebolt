...
Version Capabilities 1.8.0-next-major.4
...
```typescript
function listen('onAvailable', capability: Capability, (CapabilityInfo) => void): Promise<number>
```
...
| ------------ | ----------------------------------------------------------- | -------- | ---------------------------------------------------------------------- |
| `capability` | [`Capability`](../Capabilities/schemas/#Capability)         | true     | <br/>pattern: ^xrn:firebolt:capability:([a-z0-9\-]+)((:[a-z0-9\-]+)?)$ |
| `value`      | [`CapabilityInfo`](../Capabilities/schemas/#CapabilityInfo) | false    | Provides the capability info.                                          |
...
let listenerId = await Capabilities.listen('onAvailable', (result) => {
  console.log(result)
})
```
...
```typescript
function listen('onGranted', role: Role, capability: Capability, (CapabilityInfo) => void): Promise<number>
```
...
let listenerId = await Capabilities.listen('onGranted', (result) => {
  console.log(result)
})
```
...
```typescript
function listen('onRevoked', role: Role, capability: Capability, (CapabilityInfo) => void): Promise<number>
```
...
let listenerId = await Capabilities.listen('onRevoked', (result) => {
  console.log(result)
})
```
...
```typescript
function listen('onUnavailable', capability: Capability, (CapabilityInfo) => void): Promise<number>
```
...
let listenerId = await Capabilities.listen('onUnavailable', (result) => {
  console.log(result)
})
```
